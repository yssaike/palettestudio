    ...
    <div className={`group ${className}`}>
      ...
      >
        ...
        <div className={`
          ...
          ${textColor}
          backdrop-blur-sm bg-black/10 rounded-2xl
        `}>
          {copied ? (
            <div className="flex flex-col items-center gap-1">
              <Check size={size === 'sm' ? 16 : size === 'md' ? 20 : 24} />
              <span className="text-xs font-helvetica font-medium">Copied!</span>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1">
              <Copy size={size === 'sm' ? 14 : size === 'md' ? 18 : 22} />
              <span className="text-xs font-helvetica font-medium">Copy</span>
            </div>
          )}
        </div>
      </div>
      ...
      {showName && (
        <div className="mt-3 text-center">
          <p className="font-helvetica text-sm font-medium text-gray-700 mb-1">{color.name}</p>
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg px-3 py-2 shadow-[inset_1px_1px_2px_rgba(163,177,198,0.3),inset_-1px_-1px_2px_rgba(255,255,255,0.8)]">
            <p className="font-helvetica text-xs text-gray-600 uppercase tracking-wider font-medium">{color.hex}</p>
            <p className="font-helvetica text-xs text-gray-500 mt-1">
              RGB({color.rgb.join(', ')})
            </p>
          </div>
        </div>
